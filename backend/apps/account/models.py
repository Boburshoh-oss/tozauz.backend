...
class UserManager(BaseUserManager):
    ...
    def create_superuser(
        self, phone_number, password, first_name=None, last_name=None, categories=None
    ):
...
class User(AbstractBaseUser):
    ...
    role = models.CharField(
        max_length=50, choices=RoleOptions.choices, blank=True, null=True
    )
    categories = models.ManyToManyField(
        "packet.Category", related_name="user", blank=True
    )
    ...
    @property
    def home(self):
        """Get user's home if they are a member of one"""
        try:
            return self.home_membership.home
        except:
            return None
